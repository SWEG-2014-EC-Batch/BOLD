...
void readEmployeeRecordsAndCalculatePay();
...
int main() {
    ...
    readEmployeeRecordsAndCalculatePay();
...
void writeEmployeeRecords(const Employee employees[]) {
    ...
    for (int i = 0; i < NUM_EMPLOYEES; ++i) {
        outputFile << employees[i].id <<" "<<employees[i].sex<<" "<<employees[i].hourlyWage<<" "<<employees[i].yearsWithCompany<<endl;
     }
